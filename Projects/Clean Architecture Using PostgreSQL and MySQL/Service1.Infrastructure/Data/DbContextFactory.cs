...
namespace Service1.Infrastructure.Data
{
    public class DbContextFactory : IDbContextFactory, IDisposable
    {
        ...
        private readonly string _sqlServerDbConnectionString;
        private readonly ILogger<DbContextFactory> _logger;

        private CustomerDbContext? _customerDbContext;
        private ProductDbContext? _productDbContext;
        private CameraDbContext? _cameraDbContext;

        public DbContextFactory(string postgresqlDbConnectionString, string mysqlDbConnectionString, string sqlServerDbConnectionString, ILogger<DbContextFactory> logger)
        {
            ...
            _sqlServerDbConnectionString = sqlServerDbConnectionString ?? throw new ArgumentNullException(nameof(sqlServerDbConnectionString));
        ...
        public CameraDbContext CameraDbContext 
        {
            get 
            {
                if (_cameraDbContext == null)
                {
                    try
                    {
                        var optionsBuilder = new DbContextOptionsBuilder<CameraDbContext>();
                        optionsBuilder.UseSqlServer(_sqlServerDbConnectionString);
                        _cameraDbContext = new CameraDbContext(optionsBuilder.Options);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Failed to create CameraDbContext");
                        throw;
                    }
                }
                return _cameraDbContext;
            }
        }
        ...
        public void ApplyMigrations()
        {
            try
            {
                ...
                ApplyMigrations(CameraDbContext);
            }
            catch (Exception ex)
            {
        ...
        public void Dispose()
        {
            ...
            DisposeContext(_cameraDbContext);
        }
    }
}
